    ...
    def fichajes(elem_post):
        if elem_post.xpath('.//div[contains(concat(" ", @class, " "), " header ")][./user-link]'):
            ...
            for pc in elem_post.xpath('.//player-card'):
                print(parse_date(elem_post.xpath('.//div[@class="date"]/@title')[0]))
                trades.append({
                        'tipo': 'venta',
            ...
            for trade in trades:
                print(check_venta_excel(trade))
                if check_venta_excel(trade):
                    print(trade['jugador'])
                    insert_dato_excel(
                        trade['vendedor'],
                        5,
            ...
            # Parse the "Fichajes" section
            player_card = elem_post.xpath('.//player-card')[0]
